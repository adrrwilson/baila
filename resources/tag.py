...
class LinksItemsToTags(MethodView):
    @blp.response(201, TagSchema)
    def post(self, item_id, tag_id):
        ...
        if item.store_id != tag.store_id:
            abort(
                400,
                {"message": "Tag and Item do not belong to the same store."}
            )
...
class Tag(MethodView):
    ...
    def delete(self, tag_id):
        ...
        abort(
            400,
            {"message": "Failed to delete tag. Ensure tag is not associated with any items, then try again."}
        )